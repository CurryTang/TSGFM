...
import warnings
import numpy as np
from prompt_graph.data import load4node, node_sample_and_save, GraphDataset
...
class NodeTask(BaseTask):
      def __init__(self, data, input_dim, output_dim, graphs_list = None, normal_split = False, *args, **kwargs):
      ...
      def run(self):
            ...
            for i in range(1, 2):
                  ...
                  if not self.normal_split:
                        ...
                        idx_train = mask_to_index(self.data.train_mask)
                        idx_test = mask_to_index(self.data.test_mask)
                        train_lbls = self.data.y[idx_train]
                        test_lbls = self.data.y[idx_test]
