...
class MoleculeDataset_aug(InMemoryDataset):
    def __init__(self,
                 ...
                 dataset='zinc250k',
                 empty=False,
                 aug="none", aug_ratio=None, pcba_path='dataset/ogbg_molpcba/processed/geometric_data_processed.pt', hiv_path='dataset/ogbg_molhiv/processed/geometric_data_processed.pt'):
        """
        ...
        if not empty and self.dataset != 'pcba' and self.dataset != 'hiv':
            self.data, self.slices = torch.load(self.processed_paths[0])
        
        if self.dataset == 'pcba':
            self.data, self.slices = torch.load(pcba_path)
            delattr(self.data, 'num_nodes')
        
        elif self.dataset == 'hiv':
            self.data, self.slices = torch.load(hiv_path)
            delattr(self.data, 'num_nodes')
    ...
    def process(self):
        ...
        if self.dataset != 'pcba' and self.dataset != 'hiv':
...
def drop_nodes(data, aug_ratio):
    ...
    try:
        ...
        if getattr(data, 'edge_attr') is not None:
            data.edge_attr = data.edge_attr[edge_mask]
        else:
            data.xe = data.xe[edge_mask]
    except:
        data = data
...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
                 ...
                 pre_filter=None,
                 dataset='zinc250k',
                 empty=False, pcba_path='dataset/ogbg_molpcba/processed/geometric_data_processed.pt', hiv_path='dataset/ogbg_molhiv/processed/geometric_data_processed.pt'):
        """
        Adapted from qm9.py. Disabled the download functionality
        :param root: directory of the dataset, containing a raw and processed
        dir. The raw dir should contain the file containing the smiles, and the
        ...
        if not empty and self.dataset != 'pcba' and self.dataset != 'hiv':
            ...
            self.data, self.slices = torch.load(pcba_path)
            delattr(self.data, 'num_nodes')
            base_path = os.path.join(os.path.dirname(pcba_path), '..')
            split_file = os.path.join(base_path, 'split', 'scaffold')
            train_split = os.path.join(split_file, 'train.csv.gz')
            valid_split = os.path.join(split_file, 'valid.csv.gz')
            test_split = os.path.join(split_file, 'test.csv.gz')
            train_idx = torch.from_numpy(pd.read_csv(train_split, header = None)[0].to_numpy())
            valid_idx = torch.from_numpy(pd.read_csv(valid_split, header = None)[0].to_numpy())
            test_idx = torch.from_numpy(pd.read_csv(test_split, header = None)[0].to_numpy())
            self.splits = {'train': train_idx, 'valid': valid_idx, 'test': test_idx}
        
        elif self.dataset == 'hiv':
            self.data, self.slices = torch.load(hiv_path)
            delattr(self.data, 'num_nodes')
            base_path = os.path.join(os.path.dirname(hiv_path), '..')
            split_file = os.path.join(base_path, 'split', 'scaffold')
            train_split = os.path.join(split_file, 'train.csv.gz')
            valid_split = os.path.join(split_file, 'valid.csv.gz')
            test_split = os.path.join(split_file, 'test.csv.gz')
