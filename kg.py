...
dataset = RelLinkPredDataset(path, dataset)
...
def test():
    ...
    valid_hits = compute_hits(z, data.valid_edge_index, data.valid_edge_type)
    test_hits = compute_hits(z, data.test_edge_index, data.test_edge_type)
...
@torch.no_grad()
def compute_hits(z, edge_index, edge_type):
    ranks = []
    for i in tqdm(range(edge_type.numel())):
        ...
        rank = compute_rank(out)
        ranks.append(rank)
    all_ranking = torch.tensor(ranks, dtype=torch.float)
...
for epoch in range(1, 10001):
    ...
    if (epoch % 500) == 0:
        valid_hits, test_hits = test()
        print(f'Val hits: {valid_hits:.4f}, Test hits: {test_hits:.4f}')
