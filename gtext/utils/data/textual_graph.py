...
import gtext.utils.basics as uf
import gtext.utils.pkg.graph_utils as g_utils
from gtext.utils.data.ppr import (
    calc_approximate_ppr_rank,
    topk_approximate_ppr_matrix,
    find_top_k_neighbors_within_khop_ego_subgraph,
)
from gtext.utils.data.preprocess import load_ogb_graph_structure_only
from gtext.utils.pkg.dict2xml import dict2xml
from gtext.utils.pkg.distributed import master_process_only, process_on_master_and_sync_by_pickle
...
def _prepare_ogb_cache(
    ...
    if not os.path.exists(processed_text_file):
        if ogb_name == "ogbn-arxiv":
            from gtext.utils.data.preprocess import process_raw_arxiv
...
def preprocess_tsgfm(data_cfg: DictConfig):
    dataset = th.load(data_cfg.dataset_path, map_location="cpu")[0]
    
    g = dgl.graph((dataset.edge_index[0], dataset.edge_index[1]))
    g.ndata["feat"] = dataset.node_text_feat
...
class TextualGraph:
    ...
    def __init__(self, cfg: DictConfig):  # Process split settings, e.g. -1/2 means first split
        ...
        elif cfg.data.type == 'tsgfm':
            self.g, self.g_info, text, label_info, self.choice_to_label_id, self.choice_to_label_name = preprocess_tsgfm(
                cfg.data)
