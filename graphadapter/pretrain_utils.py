...
from graphadapter.graphadapter import GraphAdapter

def load_pretrain_graph(dataset_name):
    x = np.load(f'./token_embedding/{dataset_name}/sentence_embeddings.npy')
    data = torch.load(f'./cache_data_minilm/{dataset_name}/processed/geometric_data_processed.pt')[0]
...
def load_llm_data(dataset_name = 'instagram'):
    token_labels = np.load(f'./token_embedding/{dataset_name}/token_labels.npy', allow_pickle=True)
    token_embeddings = np.load(f'./token_embedding/{dataset_name}/token_embeddings.npy', allow_pickle=True)
    # token_embeddings = h5py.File(f'./token_embedding/{dataset_name}/token_embeddings.h5', 'r')['data']
    token_node_ids = np.load(f'./token_embedding/{dataset_name}/token_node_ids.npy', allow_pickle=True)
...
def pretrain_graph_adapter(args):
    ...
    loss_function = LabelSmoothing(50257, 0.1) # The number of categories is the number of vocabulary lists in LLM
