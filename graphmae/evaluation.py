...
import numpy as np
from graphmae.utils import create_optimizer, accuracy, show_occupied_memory, get_current_lr
import logging
from graphmae.lc_sampler import setup_finetune_dataloder, LinearProbingDataLoader, setup_eval_dataloder
import wandb
...
def linear_probing_for_inductive_node_classiifcation(model, x, labels, mask, optimizer, max_epoch, device, mute=True):
...
def linear_probing_full_batch(model, graph, x, num_classes, lr_f, weight_decay_f, max_epoch_f, device, linear_prob=True, mute=False, mode='graphmae'):
    model.eval()
    with torch.no_grad():
        if mode == 'graphmae':
            x = model.embed(graph.to(device), x.to(device))
        else:
            x = model(graph.to(device), x.to(device))
        in_feat = x.shape[1]
    encoder = LogisticRegression(in_feat, num_classes)
