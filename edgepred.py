...
from graphcl.dataloader import DataLoaderAE
from graphcl.util import NegativeEdge
...
from graphcl.splitters import scaffold_split, random_split, random_scaffold_split
import pandas as pd
...
def main():
    ...
    #set up dataset
    dataset = MoleculeDataset("dataset/dataset/" + args.dataset, dataset=args.dataset, transform = NegativeEdge())
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if not args.output_model_file == "":
            torch.save(model.state_dict(), args.output_model_file + f"{epoch}" + ".pth")
