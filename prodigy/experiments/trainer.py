...
class TrainerFS():
    def __init__(self, dataset, parameter):
        wandb.init(project=parameter["project_name"], name=parameter["exp_name"])
        #wandb.run.log_code(".")
    ...
    def _build_dataloaders(self, dataset, dataset_name):
        ...
        if dataset_name == "arxiv":
            ...
            from data.custom import get_custom_dataloader
            get_dataloader = get_custom_dataloader
    ...
    def save_checkpoint(self, step):
        ...
        hidden_dim = self.parameter['emb_dim']
        torch.save(state_dict, os.path.join(self.ckpt_dir, 'state_dict_' + str(step) + str(hidden_dim) + '.ckpt'))
    ...
    def train(self):
        ...
        for e in pbar:
            ...
            t3 = time.time()
            # wandb.log({"step_time": t3 - t2})
            # wandb.log({"load_time": t2 - t1})
            wandb.log({"train_loss": loss, "train_acc": acc, "train_aux_loss": aux_loss, "train_total_loss": total_loss})  # loss and acc here are both floats
            t_load += t2 - t1
            t_one_step += t3 - t2
            ...
            if e % self.eval_step == 0 and e != 0:
                ...
                if self.train_val_dataloader is not None:
                    with torch.no_grad():
                        ...
                        wandb.log({"train_val_loss": tval_loss, "train_val_acc": tval_acc, "train_val_aux_loss": tval_aux_loss})
