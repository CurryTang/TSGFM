...
def construct_sparse(neighbors, weights, shape):
    i = np.repeat(np.arange(len(neighbors)), np.fromiter(map(len, neighbors), dtype=np.int32))
...
if __name__ == '__main__':
    test_data = torch.load("./cache_data_minilm/cora/processed/geometric_data_processed.pt")[0]
    adj_matrix = to_scipy_sparse_matrix(test_data.edge_index).tocsr()
    alpha = 0.85
    epsilon = 1e-9
    nodes = np.arange(100)
    topk = 10
    ppr_matrix = topk_ppr_matrix(adj_matrix, alpha, epsilon, nodes, topk)
